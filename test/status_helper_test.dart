...
void main() {
  group('Expiry status', () {
    ...
    test('classifies expired expiring safe and no-expiry items', () {
      ...
      expect(calculateExpiryStatus(DateTime(2026, 5, 8), today),
          ExpiryStatus.expired);
      expect(calculateExpiryStatus(DateTime(2026, 5, 16), today),
          ExpiryStatus.expiring);
      expect(calculateExpiryStatus(DateTime(2026, 5, 17), today),
          ExpiryStatus.safe);
    ...
    test('formats visible expiry status text', () {
      ...
      expect(
          getExpiryStatusText(
              ExpiryStatus.expired, DateTime(2026, 5, 6), today),
          '过期 3 天');
      expect(
          getExpiryStatusText(
              ExpiryStatus.expiring, DateTime(2026, 5, 11), today),
          '临期 2 天');
      expect(
          getExpiryStatusText(ExpiryStatus.safe, DateTime(2026, 6, 1), today),
          '');
  ...
  group('Item display', () {
    ...
    test('formats purchase price with two decimal places', () {
      final today = DateTime(2026, 5, 9);
      final display = ItemDisplay.fromItem(
        item: Item(
          id: '1',
